...
$demonId = isset($_GET['demon_id']) ? (int)$_GET['demon_id'] : null;

if (!$demonId) {
    header('Location: /?sec=demons');
    exit;
}
  ...
  <div class="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-8 relative z-10">
    ...
    <?php if ($dbError): ?>
      <div class="text-center py-20">
        ...
        <a href="/?sec=demons" class="inline-block bg-amber-600/20 hover:bg-amber-600/30 border border-amber-600/40 text-amber-500 px-6 py-3 rounded text-sm font-bold transition-all uppercase tracking-wider">
    ...
    <div class="mx-auto max-w-7xl overflow-visible">
      ...
      <div class="mb-6 mt-2 demon-back">
        <a href="/?sec=demons" class="inline-flex items-center gap-2 px-4 py-2 rounded-md border border-amber-600/40 bg-amber-600/10 hover:bg-amber-600/20 hover:border-amber-600/60 transition-all text-amber-500 hover:text-amber-400 shadow-lg shadow-amber-500/10 hover:shadow-amber-500/20">
      ...
      <div class="flex flex-col lg:flex-row gap-8 lg:items-start">
              ...
              <?php if ($demon->stat_strength): ?>
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-amber-500 font-mono"><?= $demon->stat_strength ?>/10</span>
                  </div>
                  <div class="w-full bg-black/60 rounded-full h-2">
                    <div class="bg-amber-600 h-2 rounded-full" style="width: <?= $demon->stat_strength * 10 ?>%"></div>
                  </div>
                </div>
              <?php endif; ?>
              ...
              <?php if ($demon->stat_dexterity): ?>
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-amber-500 font-mono"><?= $demon->stat_dexterity ?>/10</span>
                  </div>
                  <div class="w-full bg-black/60 rounded-full h-2">
                    <div class="bg-amber-600 h-2 rounded-full" style="width: <?= $demon->stat_dexterity * 10 ?>%"></div>
                  </div>
                </div>
              <?php endif; ?>
              ...
              <?php if ($demon->stat_intelligence): ?>
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-amber-500 font-mono"><?= $demon->stat_intelligence ?>/10</span>
                  </div>
                  <div class="w-full bg-black/60 rounded-full h-2">
                    <div class="bg-amber-600 h-2 rounded-full" style="width: <?= $demon->stat_intelligence * 10 ?>%"></div>
                  </div>
                </div>
              <?php endif; ?>
              ...
              <?php if ($demon->stat_health): ?>
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-amber-500 font-mono"><?= $demon->stat_health ?>/10</span>
                  </div>
                  <div class="w-full bg-black/60 rounded-full h-2">
                    <div class="bg-amber-600 h-2 rounded-full" style="width: <?= $demon->stat_health * 10 ?>%"></div>
                  </div>
                </div>
              <?php endif; ?>
              ...
              <?php if ($demon->stat_reflexes): ?>
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-amber-500 font-mono"><?= $demon->stat_reflexes ?>/10</span>
                  </div>
                  <div class="w-full bg-black/60 rounded-full h-2">
                    <div class="bg-amber-600 h-2 rounded-full" style="width: <?= $demon->stat_reflexes * 10 ?>%"></div>
                  </div>
                </div>
              <?php endif; ?>
              ...
              <?php if ($demon->stat_stealth): ?>
                <div>
                  <div class="flex justify-between text-xs mb-1">
                    ...
                    <span class="text-amber-500 font-mono"><?= $demon->stat_stealth ?>/10</span>
                  </div>
                  <div class="w-full bg-black/60 rounded-full h-2">
                    <div class="bg-amber-600 h-2 rounded-full" style="width: <?= $demon->stat_stealth * 10 ?>%"></div>
                  </div>
                </div>
              <?php endif; ?>
            </div>
              ...
              <ul class="space-y-2">
                <?php foreach ($demon->personality as $trait): ?>
                  <li class="text-amber-300/90 text-sm flex items-start gap-2">
                    ...
                    <span><?= htmlspecialchars(is_string($trait) ? $trait : json_encode($trait)) ?></span>
                  </li>
                <?php endforeach; ?>
              </ul>
            <?php else: ?>
              ...
              <div class="space-y-3">
                <?php if (isset($demon->personality['core'])): ?>
                  <div>
                    <h4 class="text-amber-400 text-xs font-bold uppercase mb-1">Core</h4>
                    <p class="text-amber-300/90 text-sm"><?= htmlspecialchars($demon->personality['core']) ?></p>
                  </div>
                <?php endif; ?>
                
                <?php if (isset($demon->personality['quirks']) && is_array($demon->personality['quirks'])): ?>
                  <div>
                    <h4 class="text-amber-400 text-xs font-bold uppercase mb-1">Quirks</h4>
                    <ul class="space-y-1">
                ...
                <?php if (isset($demon->personality['toward_vassals'])): ?>
                  <div>
                    <h4 class="text-amber-400 text-xs font-bold uppercase mb-1">Hacia Vasallos</h4>
                    <p class="text-amber-300/90 text-sm"><?= htmlspecialchars($demon->personality['toward_vassals']) ?></p>
                  </div>
                <?php endif; ?>
                
                <?php if (isset($demon->personality['toward_opponents'])): ?>
                  <div>
                    <h4 class="text-amber-400 text-xs font-bold uppercase mb-1">Hacia Oponentes</h4>
                    <p class="text-amber-300/90 text-sm"><?= htmlspecialchars($demon->personality['toward_opponents']) ?></p>
                  </div>
                <?php endif; ?>
              </div>
            <?php endif; ?>
        ...
        <div class="demon-actions flex flex-col sm:flex-row gap-3">
          <a href="/?sec=pacts&demon=<?= $demon->id ?>" class="flex-1 bg-amber-600/20 hover:bg-amber-600/30 border border-amber-600/40 text-amber-500 text-sm font-bold py-3 px-6 rounded-lg transition-all text-center flex items-center justify-center gap-2">
            <i class="fa-solid fa-scroll"></i>
            VER PACTOS
          </a>
          
          <a href="/?sec=demons" class="bg-black/60 hover:bg-black/80 border border-amber-600/40 text-amber-600 hover:text-amber-500 text-sm font-bold py-3 px-6 rounded-lg transition-all text-center">
            VER MÁS DEMONIOS
          </a>
        </div>
