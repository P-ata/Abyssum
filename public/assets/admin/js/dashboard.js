// Optional JS enhancements for the dashboard (using global gsap from CDN)
(() => {
	...
	onReady(() => {
		...
		document.querySelectorAll('.pact-card').forEach(card => {
			card.addEventListener('mouseenter', () => window.gsap && gsap.to(card, { scale: 1.02, duration: 0.25, ease: 'power2.out' }));
			card.addEventListener('mouseleave', () => window.gsap && gsap.to(card, { scale: 1, duration: 0.3, ease: 'power2.out' }));
		});
		...
		if (clearBtn) {
			clearBtn.addEventListener('click', () => {
				...
				if (input) {
					input.value = '';
					if (window.gsap) gsap.fromTo(input, { opacity: 0.4 }, { opacity: 1, duration: 0.25 });
				}
			});
		}
	});
