...
<div class="min-h-screen bg-black relative overflow-hidden px-6 py-14 xl:py-16 font-mono">
  ...
  <div class="max-w-7xl mx-auto relative z-10">
    <div class="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-10 mb-14">
      <h1 id="epTitle" class="text-5xl md:text-6xl font-bold tracking-widest leading-tight text-amber-500">
      ...
      <div class="flex flex-wrap gap-3">
        ...
        if (isset($_GET['return_to'])) {
          if ($_GET['return_to'] === 'pact-detail') {
            $returnUrl = '/?sec=admin&page=pact-detail&id=' . $pact->id;
          } else {
        ...
        <form action="/?sec=admin&action=edit-pact<?= isset($_GET['return_to']) ? '&return_to=' . htmlspecialchars($_GET['return_to']) : '' ?>" method="post" enctype="multipart/form-data" class="bg-black/70 border border-amber-600/30 rounded-xl overflow-hidden" id="editPactForm">
          ...
          <div class="p-6 xl:p-8 grid gap-8 md:grid-cols-2">
            <div class="md:col-span-1 form-section">
              ...
              <select required name="demon_id" class="w-full bg-black/60 border border-amber-600/30 rounded px-4 py-2.5 text-sm text-amber-100 focus:outline-none focus:border-amber-500 transition">
                ...
                <?php foreach ($demons as $d): ?>
                  <option value="<?= $d->id ?>" <?= $d->id === $pact->demon_id ? 'selected' : '' ?>>
            ...
            <div class="md:col-span-1 form-section">
              ...
              <input required name="name" type="text" value="<?= htmlspecialchars($pact->name) ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="Overclock de Corona" />
            </div>
            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">URL (opcional)</label>
              <input name="slug" type="text" value="<?= htmlspecialchars($pact->slug ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="overclock-de-corona" />
            ...
            <div class="md:col-span-2 form-section">
              ...
              <textarea name="summary" rows="6" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="// detalles breves del pacto, condiciones, efectos, limitaciones"><?= htmlspecialchars($pact->summary ?? '') ?></textarea>
            </div>
            <div class="form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">DURACIÓN</label>
              <input name="duration" type="text" value="<?= htmlspecialchars($pact->duration ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="e.g. 1 minuto, 2h" />
            </div>
            <div class="form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">COOLDOWN</label>
              <input name="cooldown" type="text" value="<?= htmlspecialchars($pact->cooldown ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="e.g. 5 minutos" />
            </div>
            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">LIMITACIONES</label>
              <input name="limitation_1" type="text" value="<?= htmlspecialchars($limitations[0] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Limitación 1" />
              <input name="limitation_2" type="text" value="<?= htmlspecialchars($limitations[1] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Limitación 2" />
              <input name="limitation_3" type="text" value="<?= htmlspecialchars($limitations[2] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="Limitación 3" />
            </div>
            <div class="md:col-span-2 form-section">
    ...
    <div class="mt-16 text-center text-xs tracking-widest text-amber-600/50">// ADMIN :: EDIT-PACT</div>
  </div>
</div>
