		...
		<div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-12">
			<div>
				<h1 id="dashTitle" class="text-6xl font-bold tracking-widest text-amber-500">
					...
					<span class="block text-2xl mt-2 tracking-wide text-amber-600/80">// PANEL</span>
				</h1>
			</div>
		...
		<div class="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between mb-10" id="toolbar">
			<div class="flex items-center gap-3 w-full md:w-auto">
				<div class="relative flex-1 md:flex-none md:w-72">
					<input type="text" id="searchInput" class="w-full bg-black/60 border border-amber-600/30 rounded px-4 py-2 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="// buscar..." />
		...
		<?php if (!empty($demons)): ?>
			<div class="mt-16">
				<div class="flex items-center justify-between mb-8">
					...
					<a href="/?sec=admin&page=new-demon" class="px-5 py-2 rounded border border-amber-600/40 bg-black/60 hover:bg-amber-600/20 text-amber-500 text-sm tracking-wide transition">+ CREAR DEMONIO</a>
				</div>
				<div class="grid gap-6 md:grid-cols-3 lg:grid-cols-4">
					<?php foreach ($demons as $d): ?>
					<div data-searchable="<?= htmlspecialchars(strtolower($d->name . ' ' . $d->slug)) ?>" class="bg-black/70 border border-amber-600/30 rounded-xl overflow-hidden hover:border-amber-500/50 transition group">
						<!-- Image -->
