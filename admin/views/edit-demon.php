...
$demonSlug = isset($_GET['id']) ? trim($_GET['id']) : '';
  ...
  <div class="max-w-7xl mx-auto relative z-10">
    ...
    <div class="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-10 mb-14">
      <h1 id="edTitle" class="text-5xl md:text-6xl font-bold tracking-widest leading-tight text-amber-500">
        ...
        <span class="block text-xl md:text-2xl mt-3 tracking-wide text-amber-600/80">// DEMONIOS :: EDITAR</span>
      </h1>
      <div class="flex flex-wrap gap-3">
        <?php 
        $returnUrl = '/?sec=admin&page=dashboard';
        if (isset($_GET['return_to'])) {
          if ($_GET['return_to'] === 'demon-detail') {
            $returnUrl = '/?sec=admin&page=demon-detail&id=' . urlencode($demon->slug);
          } else {
            $returnUrl = '/?sec=admin&page=' . htmlspecialchars($_GET['return_to']);
          }
        }
        ?>
        <a href="<?= $returnUrl ?>" class="px-5 py-2.5 rounded border border-amber-600/30 bg-black/60 hover:bg-amber-600/20 text-amber-500 text-sm tracking-wide transition">VOLVER</a>
      </div>
    </div>
        ...
        <div class="bg-black/70 border border-amber-600/30 rounded-xl overflow-hidden" id="mediaCard">
          ...
          <div class="p-6 xl:p-7">
            <div id="dropZone" class="group relative flex flex-col items-center justify-center gap-4 border-2 border-dashed border-amber-600/30 rounded-xl bg-black/50 hover:border-amber-500/60 transition cursor-pointer p-10 text-center mb-7">
              ...
              <p class="text-amber-500 text-sm tracking-wide">Arrastrá una nueva imagen aquí</p>
            ...
            <?php if (!empty($demon->image_file_id)): ?>
        ...
        <form action="/?sec=admin&action=edit-demon<?= isset($_GET['return_to']) ? '&return_to=' . htmlspecialchars($_GET['return_to']) : '' ?>" method="post" enctype="multipart/form-data" class="bg-black/70 border border-amber-600/30 rounded-xl overflow-hidden" id="editDemonForm">
          ...
          <div class="p-6 xl:p-8 grid gap-8 md:grid-cols-2">
            ...
            <div class="md:col-span-2 form-section">
              ...
              <input name="slug" type="text" value="<?= htmlspecialchars($demon->slug) ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="aurelia" />
            ...
            <div class="md:col-span-1 form-section">
              ...
              <input name="species" type="text" value="<?= htmlspecialchars($demon->species ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="e.g. Súcubo, Íncubo" />
            </div>
            <div class="md:col-span-1 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">GÉNERO</label>
              <input name="gender" type="text" value="<?= htmlspecialchars($demon->gender ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="e.g. Femenino, Masculino" />
            </div>

            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">EDAD REAL</label>
              <input name="age_real" type="text" value="<?= htmlspecialchars($demon->age_real ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="e.g. 347 años, Milenario" />
            </div>

            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">ALIAS</label>
              <input name="alias_1" type="text" value="<?= htmlspecialchars($aliases[0] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Alias 1" />
              <input name="alias_2" type="text" value="<?= htmlspecialchars($aliases[1] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Alias 2" />
              <input name="alias_3" type="text" value="<?= htmlspecialchars($aliases[2] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="Alias 3" />
            </div>

            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">RESUMEN</label>
              <textarea name="summary" rows="4" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="// descripción breve del demonio, características principales"><?= htmlspecialchars($demon->summary ?? '') ?></textarea>
            </div>

            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">LORE</label>
              <textarea name="lore" rows="6" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="// historia completa, origen, eventos relevantes del demonio"><?= htmlspecialchars($demon->lore ?? '') ?></textarea>
            </div>

            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">PERSONALIDAD</label>
              <input name="personality_1" type="text" value="<?= htmlspecialchars($personality[0] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Rasgo 1" />
              <input name="personality_2" type="text" value="<?= htmlspecialchars($personality[1] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Rasgo 2" />
              <input name="personality_3" type="text" value="<?= htmlspecialchars($personality[2] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="Rasgo 3" />
            </div>

            <div class="md:col-span-2 form-section">
              <label class="block text-xs text-amber-600/70 tracking-widest mb-2">HABILIDADES</label>
              <textarea name="abilities_summary" rows="4" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="// resumen de habilidades y poderes especiales"><?= htmlspecialchars($demon->abilities_summary ?? '') ?></textarea>
            </div>
            ...
            <div class="md:col-span-2 form-section">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="stat_strength" type="number" min="0" max="100" value="<?= $demon->stat_strength ?? '' ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-4 py-2 text-sm text-amber-100 focus:outline-none focus:border-amber-500 transition" placeholder="0-100" />
                </div>
                <div>
            ...
            <div class="md:col-span-2 form-section">
              ...
              <input name="weakness_1" type="text" value="<?= htmlspecialchars($weaknesses[0] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Debilidad 1" />
              <input name="weakness_2" type="text" value="<?= htmlspecialchars($weaknesses[1] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition mb-2" placeholder="Debilidad 2" />
              <input name="weakness_3" type="text" value="<?= htmlspecialchars($weaknesses[2] ?? '') ?>" class="w-full bg-black/60 border border-amber-600/30 rounded px-5 py-3 text-sm text-amber-100 placeholder:text-amber-600/40 focus:outline-none focus:border-amber-500 transition" placeholder="Debilidad 3" />
            </div>
          </div>
          <div class="px-6 xl:px-8 py-5 border-t border-amber-600/20 flex flex-wrap items-center justify-end gap-4">
            <a href="<?= isset($_GET['return_to']) ? '/?sec=admin&page=' . htmlspecialchars($_GET['return_to']) : '/?sec=admin&page=dashboard' ?>" class="px-5 py-2 rounded border border-amber-600/30 bg-black/60 hover:bg-amber-600/20 text-amber-500 text-sm tracking-wide transition">CANCELAR</a>
    ...
    <div class="mt-16 text-center text-xs tracking-widest text-amber-600/50">// ADMIN :: EDIT-DEMON</div>
  </div>
</div>
